...
test("server-renders the complete BiliFlow customizer", async () => {
  ...
  assert.match(html, /Enhanced 3\.9\.1/);
  assert.match(html, /Bilibili iOS 9\.6\.1/);
  ...
  assert.match(html, /普通视频身份字段不完整导致的首页空流/);
  assert.match(html, /后台定时测速（推荐）/);
  assert.match(html, /生成重置令牌/);
  assert.match(html, /一键安装到 Shadowrocket/);
  assert.match(html, /明确的安全边界/);
  assert.doesNotMatch(html, /Enhanced (?:3\.8\.[012]|3\.9\.0)|CDN v8|v7 对象级缓存/);
  assert.doesNotMatch(html, /Bilibili iOS 9\.5\.0|9\.5\.0 Story\/搜索广告/);
