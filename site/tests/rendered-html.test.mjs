...
test("server-renders the complete BiliFlow customizer", async () => {
  ...
  assert.match(html, /Enhanced 3\.9\.3/);
  assert.match(html, /Bilibili iOS 9\.7\.0/);
  ...
  assert.match(html, /首页非空保护/);
  assert.match(html, /后台定时测速（推荐）/);
  assert.match(html, /生成重置令牌/);
  assert.match(html, /一键安装到 Shadowrocket/);
  assert.match(html, /明确的安全边界/);
  assert.doesNotMatch(html, /Enhanced (?:3\.8\.[012]|3\.9\.[012])|CDN v8|v7 对象级缓存/);
  assert.doesNotMatch(html, /Bilibili iOS 9\.[56]\.\d|9\.5\.0 Story\/搜索广告/);
