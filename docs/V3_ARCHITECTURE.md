...
首页 App JSON 不再硬性要求同时具备 AVID、BVID、CID、URI 或固定卡型：已知普通
卡型是 AV 类型的正向证据，未知新卡型有明确 AV/video 类型与 AVID、BVID、
...
- 未知未来服务入口默认显示；未知未来 Protobuf 字段默认保留。播放页关系卡的
  未知类型在 `推荐仅普通视频=true` 时删除；首页未知卡型若有明确 AV 类型和视频
  身份则保留，否则由非空 fallback 防止整页被过滤为空。
