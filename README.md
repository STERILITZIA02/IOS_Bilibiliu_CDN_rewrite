...
>
> v3.7.0 针对 Bilibili iOS 9.5.0 的 Mine 大会员横幅、后台响应表示绕过与
> 冷门视频 CDN 前缀误判所做的增量修复、证据边界及回滚方式见
> [v3.7 增量审计](docs/V3_7_AUDIT.md)。上一轮完整 9.5.0 广告/CDN 审计仍保留在
> [v3.6 审计记录](docs/V3_6_AUDIT.md)。
...
`/x/v2/feed/index/story` 与其 `/cart` 异步购物响应使用同一个
...
接口、`/x/v2/feed/index`、`/x/v2/feed/index/story(/cart)`、`/x/v2/view`、
...
| `测速间隔` | `12` | 选择缓存 TTL 为 6–72 小时；已选线路最多每 8 分钟非阻塞复核，所有探测仍至少间隔 2 分钟 |
...
6. 首次探索对主备各取相同的 256 KiB 前缀，降低起播时的额外带宽竞争；第二次确认及
   后续健康复核改取相同的 1 MiB 文件内部区间，并在中部、四分之一和四分之三附近
   轮转。双方均必须为 `206`，Range 起止、总文件长度、实际样本长度和内容样本 hash
   完全一致，Content-Type 兼容，且没有内容压缩、跨对象重定向或
   HTML/JSON/XML/错误页；
7. 以文件内部 1 MiB 样本吞吐中位数为主、总耗时与抖动/失败率为保护条件；备用路线除达到
   相对切换阈值外，还必须满足音频、视频或分段的绝对吞吐下限，以及声明
   `bandwidth` 的 1.35 倍余量；新备用路线需至少间隔 2 分钟成功两次；
8. 探测结果只影响之后重新取得的播放地址，不中断当前播放；
9. 使用当前响应中同一 alias lane 的完整签名 URL；camelCase/snake_case 的签名
   不互换，不复用旧 query，不删除原始备用；
10. v7 选择状态始终绑定到当前媒体对象的精确无查询路径，并额外隔离媒体种类、表示、
    编码、网络档案和当前候选集合；绝不把上一条视频的已选主机晋升到下一条视频；
11. 已选线路最多每 8 分钟做一次非阻塞健康复核；超时、403、样本不一致或已不再
    更快时清除选择，下一份响应恢复服务端原始 URL。用户选择的 6–72 小时仍是
    精确选择 TTL，不会被健康复核静默缩短；
12. v7 额外维护最多 48 个**仅由候选 hash 标识**的失败/低速熔断记录；它只用于
    暂时跳过当前响应中已经存在的坏备用 URL，绝不把别的视频 URL、主机选择或签名
    迁移过来。硬失败立即退避 15 分钟并指数延长，连续两次低于当前表示吞吐要求也
    会暂时熔断；
13. 持久化不保存媒体路径、主机名、完整 URL、query 或 token，只保存 hash、
    摘要、计数和时间戳，选择最多 64 条、主机健康最多 48 条；任何异常都原样放行。
...
“第一次就选最低延迟”。
升级到 3.7.0 时旧的 `safeAuto.v6` 学习结果不会迁移，避免继续使用只验证文件
前缀的误选线路；v7 从每个媒体对象的服务端原始 URL 安全重新学习。
...
BiliFlow 生成的固定 URL，升级到 3.7.0 **不需要重新订阅**，只需执行上述“更新
模块”。更新后模块详情应显示 `3.7.0`，脚本 URL 应含 `?v=3.7.0`。只有把 Release
