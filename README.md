...
>
> v3.5.0 针对 Bilibili iOS 9.5.0 的 Story 广告/CDN 旁路、搜索商业卡、
> 直播购物弹层和跨视频 CDN 缓存问题的确认链路、证据边界及回滚方式见
> [增量审计记录](docs/V3_5_AUDIT.md)。
...
schema 原样保留。字段缺失或无法确认商业语义时不依据标题猜测。
...
10. v5 状态始终绑定到当前媒体对象的精确无查询路径，并额外隔离媒体种类、表示、
...
“第一次就选最低延迟”。
升级到 3.5.0 时旧的 `safeAuto.v4` 学习结果不会迁移，避免继续套用跨视频复用
的线路质量；v5 从每个媒体对象的服务端原始 URL 安全重新学习。
...
BiliFlow 生成的固定 URL，升级到 3.5.0 **不需要重新订阅**，只需执行上述“更新
模块”。更新后模块详情应显示 `3.5.0`，脚本 URL 应含 `?v=3.5.0`。只有把 Release
