...
6. 排名只考虑本次媒体对象可用的目标。无合格状态时，只有当前响应含完整 Akamai
   备用 URL、且没有新鲜失败/带宽不足证据时才提升它；否则保留服务端原主地址。
   冷启动回退不再绕过熔断和已知质量门槛，原主地址仍保留在备选中；
...
损坏、过期、对象或档案不一致时原地址直接放行；请求侧还会读取最新的主机熔断状态。
同一对象的新播放响应决定回到原主地址时会撤销旧重定向，避免旧缓存覆盖新的选择。

首次安装无需等待学习：有完整 Akamai 备用且没有已知失败证据时使用冷启动回退。后台 cron 在至少
