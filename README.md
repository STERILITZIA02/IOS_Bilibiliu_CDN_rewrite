...
| **CDN + Enhanced** | CDN Switcher 的全部能力，以及首页六条普通视频流、播放页普通视频推荐白名单、广告过滤、首页/“我的”逐项精简 | 希望清理广告、非普通视频推荐和不需要的服务入口 |
...
字段；“我的”页的大会员横幅还必须命中明确横幅结构与会员营销上下文。以下内容
属于保护边界：会员状态、到期时间、钱包、订单、付款渠道、权益列表和未知字段。
...
BiliFlow 生成的固定 URL，升级到 3.1.0 **不需要重新订阅**，只需执行上述“更新
模块”。更新后模块详情应显示 `3.1.0`，脚本 URL 应含 `?v=3.1.0`。只有把 Release
