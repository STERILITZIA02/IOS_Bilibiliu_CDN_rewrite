...
>
> v3.6.0 针对 Bilibili iOS 9.5.0 的持续吞吐误判、跨视频失败节点重试、
> 搜索运营词与漫画闪屏入口所做的增量修复、实测边界及回滚方式见
> [v3.6 增量审计](docs/V3_6_AUDIT.md)。上一轮 Story/搜索/直播审计仍保留在
> [v3.5 审计记录](docs/V3_5_AUDIT.md)。
...
6. 固定使用 `GET Range: bytes=0-1048575`（1 MiB），要求双方均为 `206`，Range 起止、总文件
   长度、实际样本长度和内容样本 hash 完全一致，Content-Type 兼容，且没有内容
   压缩、跨对象重定向或 HTML/JSON/XML/错误页；
7. 以 1 MiB 样本吞吐中位数为主、总耗时与抖动/失败率为保护条件；备用路线除达到
...
10. v6 选择状态始终绑定到当前媒体对象的精确无查询路径，并额外隔离媒体种类、表示、
...
12. v6 额外维护最多 48 个**仅由候选 hash 标识**的失败/低速熔断记录；它只用于
...
“第一次就选最低延迟”。
升级到 3.6.0 时旧的 `safeAuto.v5` 学习结果不会迁移，避免继续使用短样本误选
的线路；v6 从每个媒体对象的服务端原始 URL 安全重新学习。
...
BiliFlow 生成的固定 URL，升级到 3.6.0 **不需要重新订阅**，只需执行上述“更新
模块”。更新后模块详情应显示 `3.6.0`，脚本 URL 应含 `?v=3.6.0`。只有把 Release
