...
| **CDN + Enhanced** | CDN Switcher 的全部能力，以及广告过滤、播放页普通视频推荐白名单、首页/“我的”逐项精简 | 希望清理广告、非普通视频推荐和不需要的服务入口 |
...
- 每次 Shadowrocket 更新该 URL 时，重新取得仓库最新模块并保留选择；
- 只访问两个固定模块源，不接受用户提供的远程脚本地址；
- 拒绝未知参数、重复参数、越界数字、换行/配置注入和不安全主机。

网站不需要账号、数据库、Cookie、分析脚本，也不收集 Bilibili 或
Shadowrocket 数据。离线内置目录只用于 GitHub 暂时不可用时展示界面；最新模块
无法取得或结构校验失败时，生成接口会失败关闭，不会下发未知脚本。
...
| 参数 | 默认 | 行为 |
| --- | --- | --- |
| `广告过滤` | `true` | 只过滤明确广告字段、已审核类型或多项商业特征命中的卡片 |
...
头像、搜索、消息/红点、首页、关注、“我的”、普通视频推荐和未知新**导航入口**
始终不作为清理目标。仅播放页推荐列表采用上面的普通 AV 白名单；其他页面不会
用静态白名单覆盖服务器返回结果。
...
2. 关闭 `广告过滤`、`推荐仅普通视频`、`界面精简`、`搜索推广`、`直播带货`、
   `会员营销`；
